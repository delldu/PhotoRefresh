...
class ScratchDataset(data.Dataset):
    """Define dataset."""

    def __init__(self, root, transforms=get_transform()):
        """Init dataset."""
        super(ScratchDataset, self).__init__()
...
def train_data(bs):
    ...
    train_ds = ScratchDataset(train_dataset_rootdir, get_transform(train=True))
    print(train_ds)
...
def test_data(bs):
    ...
    test_ds = ScratchDataset(test_dataset_rootdir, get_transform(train=False))
...
def scratchdetectDatasetTest():
    """Test dataset ..."""

    ds = ScratchDataset(train_dataset_rootdir)
    print(ds)
    # src, tgt = ds[0]
